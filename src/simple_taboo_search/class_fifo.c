 ...
 * key is a "distance" (represented by an integer) and not and entire graph
 */
void *FIFOInitEdge(int size)
{
	return(FIFOInitGraph(size));
}

void
FIFODeleteEdge(void *in_f)
{
	FIFO_t *f = (void *)in_f;
	Dllist dl;
	JRB jnode;
...
double MakeEdgeKey(int distance, int count)
{
	double key;
...
int
FIFOInsertEdge(void *in_f, int distance, int count)
{
	FIFO_t *f = (void *)in_f;
	JRB jnode;
	double dkey;
	...
	key = MakeEdgeKey(distance,count);
	...
	if((f->fcount+1) > f->max_size)
	{
		...
		{
			fprintf(stderr,
		"FIFOInsertEdge error: key %f not in index\n",
					dkey);
			fflush(stderr);
		}
		else
...
int FIFOFindEdge(void *in_f, int distance, int count) 
{
	FIFO_t *f = (void *)in_f;
	JRB jnode;
	double key;
